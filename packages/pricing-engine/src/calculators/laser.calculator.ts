...
export class LaserPricingCalculator extends BasePricingCalculator {
  ...
  calculateMaterialUsage(): MaterialUsage {
    ...
    const partAreaCm2 = geometry.surfaceAreaCm2 || 
      (geometry.bboxMm.x * geometry.bboxMm.y) / 100;
  ...
  private interpolateSpeed(speeds: Record<number, number>, thickness: number): number {
    const thicknesses = Object.keys(speeds).map(Number).sort((a, b) => a - b);
    ...
    return speeds[lowerThickness] + 
      ratio * (speeds[upperThickness] - speeds[lowerThickness]);
  }

  private interpolatePierceTime(thickness: number): number {
    const thicknesses = Object.keys(this.PIERCE_TIMES).map(Number).sort((a, b) => a - b);
    
    for (let i = 0; i < thicknesses.length - 1; i++) {
      if (thickness >= thicknesses[i] && thickness <= thicknesses[i + 1]) {
        const ratio = (thickness - thicknesses[i]) / (thicknesses[i + 1] - thicknesses[i]);
        return this.PIERCE_TIMES[thicknesses[i]] + 
  ...
  private generateWarnings(usage: MaterialUsage, time: ProcessingTime): string[] {
    ...
    // Large sheet warning
    if (usage.grossVolumeCm3 / (thickness / 10) > 10000) { // > 1m²
