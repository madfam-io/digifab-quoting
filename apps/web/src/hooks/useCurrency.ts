...
import { Currency, formatCurrency, getCurrencySymbol, getCurrencyName, CURRENCY_CONFIG } from '@cotiza/shared';
...
export function useCurrency(): UseCurrencyReturn {
  ...
  const fetchRates = useCallback(async (base: Currency = currency): Promise<ExchangeRates> => {
  ...
  const loadRates = useCallback(async (base: Currency = currency): Promise<void> => {
  ...
  const setCurrency = useCallback(async (newCurrency: Currency): Promise<void> => {
  ...
  const symbol = useCallback((targetCurrency: Currency = currency): string => {
    return getCurrencySymbol(targetCurrency);
  }, [currency]);

  const name = useCallback((targetCurrency: Currency = currency): string => {
