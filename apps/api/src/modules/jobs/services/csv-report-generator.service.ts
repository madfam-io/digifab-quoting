...
export class CsvReportGeneratorService {
  ...
  private generateQuoteOrderCsv(data: any, reportType: 'quote' | 'order'): string {
    ...
    if (items && items.length > 0) {
      ...
      items.forEach((item: any, index: number) => {
        const fileName = this.escapeCsvValue(item.files?.[0]?.originalName || item.name || 'Unknown');
        const material = this.escapeCsvValue(item.material?.name || 'N/A');
        const process = this.escapeCsvValue(item.manufacturingProcess?.name || item.processCode || 'N/A');
        const total = item.unitPrice * item.quantity;
        
        lines.push(`${index + 1},${fileName},${material},${process},${item.quantity},${item.unitPrice},${total}`);
      });
    }
  ...
  private generateInvoiceCsv(invoice: any): string {
    ...
    if (invoice.customer) {
      ...
      if (invoice.customer.billingAddress) {
        ...
        lines.push(`Address,"${this.escapeCsvValue([
          addr.street,
          addr.city,
          addr.state,
          addr.postalCode,
  ...
  private generateAnalyticsCsv(data: any): string {
    ...
    lines.push(`Period,${new Date(data.criteria.startDate).toLocaleDateString()} - ${new Date(data.criteria.endDate).toLocaleDateString()}`);
    lines.push('');
    ...
    if (data.revenue && data.revenue.length > 0) {
      ...
      data.revenue.forEach((period: any) => {
        lines.push(`${new Date(period.period).toLocaleDateString()},${period.order_count},${period.revenue}`);
      });
    }
