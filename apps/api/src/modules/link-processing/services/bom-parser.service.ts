...
export class BOMParserService {
  ...
  private parseInstructablesBOM(content: RawContent): BOMItemDto[] {
    ...
    supplies.forEach((supply: any) => {
      const item = this.createBOMItem(
        supply.name,
        parseInt(supply.quantity) || 1,
        'component'
      );
  ...
  private categorizeComponent(name: string): string {
    ...
    if (nameLower.includes('arduino') || nameLower.includes('raspberry') || nameLower.includes('microcontroller')) {
      return 'electronics';
    }
    if (nameLower.includes('resistor') || nameLower.includes('capacitor') || nameLower.includes('led')) {
      return 'electronics';
    }
  ...
  private suggestMaterial(item: BOMItemDto): string {
    if (item.manufacturingMethod === ProcessType.PRINTING_3D_FFF) {
      ...
      if (item.name.toLowerCase().includes('outdoor') || item.name.toLowerCase().includes('strong')) {
        return 'PETG';
      }
      return 'PLA';
    }
  ...
  private isCommonWord(word: string): boolean {
    const commonWords = ['the', 'and', 'for', 'you', 'with', 'this', 'that', 'have', 'need', 'make', 'using'];
