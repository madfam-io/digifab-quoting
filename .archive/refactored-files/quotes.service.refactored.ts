...
export class QuotesService {
  ...
  async create(tenantId: string, customerId: string, dto: CreateQuoteDto): Promise<Quote> {
    try {
      ...
      const existingDraft = await this.quoteRepository.findDraftQuote(customerId, tenantId);
  ...
  async addItem(tenantId: string, quoteId: string, dto: AddQuoteItemDto): Promise<QuoteItem> {
    try {
  ...
  async calculate(tenantId: string, quoteId: string, dto: CalculateQuoteDto): Promise<any> {
    try {
      ...
      // Calculate pricing
      const result = await this.calculationService.calculateQuote(quoteId, tenantId, dto.items);

      // Update quote items and totals
      await this.quoteRepository.transaction(async (tx) => {
        // Batch update all items
        await this.quoteRepository.batchUpdateItems(quoteId, tenantId, result.itemsToUpdate);

        // Update quote with totals and status
        await tx.quote.update({
          where: { id: quoteId },
          data: {
            status: result.warnings.length > 0 ? QuoteStatus.NEEDS_REVIEW : QuoteStatus.AUTO_QUOTED,
            ...
            sustainability: this.calculateSustainabilitySummary(result.itemsToUpdate),
          },
        });
      });
  ...
  async cancel(tenantId: string, quoteId: string, reason?: string): Promise<Quote> {
    const quote = await this.findOne(tenantId, quoteId);
  ...
  async expire(tenantId: string): Promise<number> {
    ...
    for (const quote of expiredQuotes) {
      try {
        await this.quoteRepository.updateStatus(quote.id, tenantId, QuoteStatus.EXPIRED);
        count++;
      } catch (error) {
  ...
  private canUpdateQuote(status: QuoteStatus): boolean {
    return [QuoteStatus.DRAFT, QuoteStatus.SUBMITTED, QuoteStatus.NEEDS_REVIEW].includes(status);
  }

  private canAcceptQuote(status: QuoteStatus): boolean {
    return [QuoteStatus.QUOTED, QuoteStatus.AUTO_QUOTED].includes(status);
  }

  private canCancelQuote(status: QuoteStatus): boolean {
    return ![QuoteStatus.ACCEPTED, QuoteStatus.CANCELLED, QuoteStatus.EXPIRED].includes(status);
  }
