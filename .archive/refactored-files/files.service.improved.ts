...
export class FilesService {
  ...
  async getFileUrl(tenantId: string, fileId: string, expiresIn: number = 3600): Promise<string> {
    const file = await this.findOne(fileId, tenantId);
  ...
  async batchDelete(tenantId: string, fileIds: string[]): Promise<void> {
    ...
    try {
      ...
      await Promise.all(fileIds.map((id) => this.clearFileCache(id, tenantId)));
  ...
  private validateFileUpload(
    ...
    if (fileSize > maxSize * 1024 * 1024) {
      throw new BadRequestException(`File size exceeds maximum of ${maxSize}MB`);
    }
    ...
    if (!extension || !allowedExtensions.includes(extension)) {
      throw new BadRequestException(`Invalid file extension .${extension} for type ${fileType}`);
    }
  ...
  // Cache management
  private async cacheFileMetadata(fileId: string, tenantId: string, metadata: any): Promise<void> {
    const cacheKey = `file:${tenantId}:${fileId}`;
    await this.cacheService.set(cacheKey, metadata, 3600); // 1 hour
  }

  private async getCachedFileMetadata(fileId: string, tenantId: string): Promise<any> {
    const cacheKey = `file:${tenantId}:${fileId}`;
    return this.cacheService.get(cacheKey);
  }

  private async clearFileCache(fileId: string, tenantId: string): Promise<void> {
    const keys = [`file:${tenantId}:${fileId}`, `file-url:${fileId}`];
