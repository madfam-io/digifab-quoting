...
async function startService(service) {
  ...
  try {
    ...
    if (err.message.includes('command not found') || err.message.includes('No such file')) {
      ...
      log.info(`To install ${service.name}, run: ${colors.cyan}${service.installCommand}${colors.reset}`);
    }
    ...
    try {
      ...
      if (err.message.includes('migrations to apply')) {
        ...
        const answer = await new Promise((resolve) => {
          rl.question(`${colors.yellow}Would you like to run migrations now? (y/n): ${colors.reset}`, resolve);
        });
        rl.close();
